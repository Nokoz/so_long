...
/*   Updated: 2023/09/11 15:03:41 by gvardaki         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (i < g->map.y)
	{
		...
		while (j <= g->map.x)
		{
			if (g->map.map[i][j] == '1')
				...
				mlx_put_image_to_window(g->mlx_ptr, g->win_ptr, g->floor.ptr, j * W, i * H);
			else if (g->map.map[i][j] == 'C')
				mlx_put_image_to_window(g->mlx_ptr, g->win_ptr, g->collectible.ptr, j * W, i * H);
